
import { Language, CreditsTranslations } from './types';

export const creditsTranslations: Record<Language, CreditsTranslations> = {
  fr: {
    buyCredits: 'Acheter des crédits',
    currentCredits: 'Vos crédits',
    creditsExplainer: 'Les crédits vous permettent de générer des designs d\'ongles. Chaque design coûte 1 crédit.',
    lowCredits: 'Crédits faibles!',
    notEnoughCredits: 'Pas assez de crédits',
    processing: 'Traitement...',
    success: 'Succès!',
    mostPopular: 'Plus populaire',
    fewMoreDrops: 'Encore quelques gouttes de vernis...',
    costOneCredit: 'Coûte 1 crédit',
    inviteFriends: 'Inviter des amis',
    inviteExplainer: 'Invitez vos amis et recevez chacun 5 crédits gratuits lorsqu\'ils s\'inscrivent!',
    generateInvite: 'Générer un lien d\'invitation',
    copyCode: 'Copier le code',
    shareInvite: 'Partager l\'invitation',
    inviteCodeCopied: 'Code d\'invitation copié!',
    enterInviteCode: 'Avez-vous un code d\'invitation?',
    invalidInviteCode: 'Code d\'invitation invalide',
    successInviteCode: 'Code d\'invitation validé! Vous recevez 5 crédits!',
  },
  en: {
    buyCredits: 'Buy Credits',
    currentCredits: 'Your Credits',
    creditsExplainer: 'Credits allow you to generate nail designs. Each design costs 1 credit.',
    lowCredits: 'Low credits!',
    notEnoughCredits: 'Not enough credits',
    processing: 'Processing...',
    success: 'Success!',
    mostPopular: 'Most Popular',
    fewMoreDrops: 'Just a few more drops of polish...',
    costOneCredit: 'Costs 1 credit',
    inviteFriends: 'Invite Friends',
    inviteExplainer: 'Invite your friends and both get 5 free credits when they sign up!',
    generateInvite: 'Generate Invitation Link',
    copyCode: 'Copy Code',
    shareInvite: 'Share Invitation',
    inviteCodeCopied: 'Invitation code copied!',
    enterInviteCode: 'Have an invitation code?',
    invalidInviteCode: 'Invalid invitation code',
    successInviteCode: 'Invitation code validated! You received 5 credits!',
  }
};
